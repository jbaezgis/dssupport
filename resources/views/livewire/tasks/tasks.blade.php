            ...
            <tbody>
                @foreach ($tasks as $task)
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            <x-alinks.secondary href="{{ url('tasks/'.$task->name)}}">{{__('Tasks')}} </x-alinks.secondary>
                            <x-buttons.primary wire:click="edit({{$task->id}})">{{__('Edit')}} </x-buttons.primary>
                            <x-buttons.danger wire:click="delete({{$task->id}})">{{__('Delete')}} </x-buttons.danger>
                        </td>
